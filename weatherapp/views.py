...
def _get_clock_and_date(tz_offset=None):
    ...
    time = {"clock": clock, "date": date, "current_dt": current_dt}
    return time
...
def _get_forecast_response(city_name="Jersey City",
    ...
    for forecast in forecast_response["list"]:
        dt = pd.to_datetime(forecast["dt_txt"]) + pd.Timedelta(timezone_offset,
                                                               unit="seconds")
        df = pd.DataFrame({
            "DATETIME": [dt],
...
def _process_forecast_response(df):
    df["DATE"] = df["DATETIME"].dt.strftime("%Y-%m-%d")
    df["LO"] = df.groupby("DATE")["LOCAL_MIN_TEMP"].transform("min").round(
        0).astype(int)
    df["HI"] = df.groupby("DATE")["LOCAL_MAX_TEMP"].transform("max").round(
        0).astype(int)
    ...
    df["HIGHEST_PRIORITY"] = df.groupby(["DAY", "PERIOD"
    ...
    df = df[["DATE", "DAY", "HI", "LO", "PERIOD", "CONDITION",
             "ICON"]].drop_duplicates()

    df = df.set_index(["DATE", "DAY", "HI", "LO",
                       "PERIOD"]).unstack("PERIOD").reset_index()
    df.columns = [t[1] + "_" + t[0] if t[1] else t[0] for t in df]

    col_order = [
        "DATE", "DAY", "HI", "LO", "MORNING_CONDITION", "NOON_CONDITION",
        "EVENING_CONDITION", "MORNING_ICON", "NOON_ICON", "EVENING_ICON"
    ]
    df = df[col_order]
...
def index(request):
    ...
    weather_response = _get_weather_response(city_name=city_name,
    ...
    time = _get_clock_and_date(tz_offset=weather["timezone_offset"])

    df = _get_forecast_response(city_name=city_name,
                                state_code=state_code,
                                unit=unit)

    df = _process_forecast_response(df)
    # IF TODAY IS MISSING when the data rolls, just append current weather
    # to the front of the forecast df.
    today = time["current_dt"].strftime("%a").upper()
    if df["DAY"].tolist()[0] != today:
        df_today = pd.DataFrame(columns=df.columns,
                                data=[[
                                    time["current_dt"], today, weather["hi"],
                                    weather["lo"], "NONE", "NONE", "NONE",
                                    None, None, weather["icon"]
                                ]])
    ...
    context = {"weather": weather, "time": time, "forecast": weather_forecast}
